...
import { AiResultModal } from "../../../components/dialogs/ai-result-modal"
import {
  AgentsFileMention,
...
export function NewChatForm({
  ...
  // AI query for worktree setup
  const { queryAi, isLoading: isGeneratingWorktree } = useAiQuery()
  const [showWorktreeAiModal, setShowWorktreeAiModal] = useState(false)
  const [aiGeneratedConfig, setAiGeneratedConfig] = useState("")

  // AI query for slash commands
  const [slashCommandResult, setSlashCommandResult] = useState<{
    title: string
    content: string
  } | null>(null)
  ...
  const handleSlashCommandWithAi = async (
    commandName: string,
    prompt: string
  ) => {
    const result = await queryAi(prompt, { model: "sonnet" })

    if (result.success && result.text) {
      // Map command names to user-friendly titles
      const titles: Record<string, string> = {
        review: "Code Review",
        "pr-comments": "PR Review Comments",
        "release-notes": "Release Notes",
        "security-review": "Security Audit",
        "worktree-setup": "Worktree Configuration",
      }

      setSlashCommandResult({
        title: titles[commandName] || "AI Response",
        content: result.text,
      })
    }
  }
  ...
  return (
    <div className="flex h-full flex-col">
                  ...
                  <div className="absolute left-0 right-0 top-full mt-2 ml-[5px] mr-[5px] p-3 pb-4 bg-muted/50 rounded-lg border border-border space-y-3">
                    ...
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <Button
                        ...
                        disabled={isGeneratingWorktree}
                      >
                        {isGeneratingWorktree ? "Generating..." : "Fill with AI"}
                      </Button>
                    </div>
                  </div>
                )}
      ...
      {/* AI Result Modal for worktree setup */}
      <AiResultModal
        ...
        onAccept={(config) => {
          // For now, just copy to clipboard since the modal is in new-chat-form
          // The actual population should happen in the settings dialog
          toast.success("Config copied to clipboard - open Settings to paste")
      ...
      {/* AI Result Modal for slash commands */}
      <AiResultModal
        open={!!slashCommandResult}
        onOpenChange={(open) => !open && setSlashCommandResult(null)}
        title={slashCommandResult?.title || ""}
        content={slashCommandResult?.content || ""}
        showCopy={true}
      />
    </div>
  )
