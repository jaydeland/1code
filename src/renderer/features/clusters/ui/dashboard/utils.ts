import type { K8sPod, K8sPvc, K8sDeployment, K8sService } from "../../../../../main/lib/kubernetes/kubernetes-service"
...
export function calculateClusterStats(
  ...
  const pvcStats = {
    total: pvcs?.length || 0,
    bound: pvcs?.filter((pvc) => pvc.status === "Bound").length || 0,
    pending: pvcs?.filter((pvc) => pvc.status === "Pending").length || 0,
    lost: pvcs?.filter((pvc) => pvc.status === "Lost").length || 0,
  }
