...
function extractBackgroundTasks(content: string): BackgroundTaskMetadata[] {
  ...
  if (bgTaskMatch && !tasks.some(t => t.type === 'background-tasks')) {
    ...
    const taskNamesPattern = /(?:docker\s+compose|stern|ktop|monitor\.sh)/gi
...
export const workflowsRouter = router({
  ...
  readFileContent: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      // This allows ~/.claude/, <project>/.claude/, and custom plugin directories
   ...
   * Emits an event when the file content changes
   */
  watchFile: publicProcedure
    .input(z.object({ path: z.string() }))
    .subscription(async ({ input }) => {
      return observable<{ changed: boolean }>((emit) => {
        let watcher: any = null

        // Dynamically import chokidar (ESM only)
        import("chokidar").then(({ default: chokidar }) => {
          const resolvedPath = path.resolve(input.path)
          console.log('[workflows.watchFile] Watching:', resolvedPath)
          ...
          watcher.on('change', () => {
            console.log('[workflows.watchFile] File changed:', resolvedPath)
            emit.next({ changed: true })
          })

          watcher.on('error', (error: Error) => {
            console.error('[workflows.watchFile] Watch error:', error)
            emit.error(error)
          })
        }).catch((error) => {
          console.error('[workflows.watchFile] Failed to initialize watcher:', error)
          emit.error(error)
        })
        ...
        return () => {
          if (watcher) {
            console.log('[workflows.watchFile] Stopping watch:', input.path)
            watcher.close()
          }
        }
      })
