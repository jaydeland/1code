...
function spawnPty(params: {
	...
	// Log diagnostic info
	console.log(`[Terminal] Spawning PTY: shell=${resolvedShell}, cwd=${resolvedCwd}`)
	console.log(`[Terminal] PATH length: ${env.PATH?.length || 0} chars`)
	console.log(`[Terminal] HOME: ${env.HOME}`)
	console.log(`[Terminal] Total env vars: ${Object.keys(env).length}`)
	console.log(`[Terminal] Shell args:`, shellArgs)

	// Check file descriptors
	if (process.platform === "darwin") {
		console.log(`[Terminal] process.pid: ${process.pid}`)
	}
